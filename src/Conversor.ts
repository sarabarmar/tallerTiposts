...
function convertirUnidad(valor: number, desde: Unidad, hacia: Unidad): number | string {
  if (valor < 0) return "El valor no puede ser negativo";

  const factores: { [key in Unidad]: number } = {
    cm: 0.01,
    m: 1,
    km: 1000,
    in: 0.0254,
  ...
  if (!(desde in factores) || !(hacia in factores)) return "Unidad no válida";
